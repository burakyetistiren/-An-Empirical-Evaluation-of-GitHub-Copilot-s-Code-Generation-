...
def calculate_different_correctness_cases(path):
    ...
    for i in range(1, NUMBER_OF_SAMPLES + 1):
        if not '-1' in matrix[i][2]:
            if float(matrix[i][2]) == 1.0:
                count_correct += 1
            elif float(matrix[i][2]) == 0.0:
                count_incorrect += 1
            else:
...
def calculate_correctness_by_percentage(path):
    ...
    count_0_25 = 0
    for i in range(1, NUMBER_OF_SAMPLES + 1):
        if 1 > float(matrix[i][2]) > 0.75:
            count_75_100 += 1
        elif 0.75 >= float(matrix[i][2]) > 0.5:
            count_50_75 += 1
        elif 0.5 >= float(matrix[i][2]) > 0.25:
            count_25_50 += 1
        elif 0.25 >= float(matrix[i][2]) > 0:
            count_0_25 += 1
...
def return_percentages_correctness(path):
    ...
    for i in range(len(scale_ratings)):
        percentages_scale_ratings.append(float(scale_ratings[i] / sum(scale_ratings)))
